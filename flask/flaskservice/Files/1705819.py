...
class _1705819():
    ...
    def __tranformation1(self,data):
        ...
        mapper = data.groupby('customerName')['invoiceAge'].mean().to_dict()
        data['age_invoice_y'] = data['customerName'].map(mapper)
        return data
    ...
    def getPredictions(self,data,model):
        ...
        features = ['customerNumber', 'invoiceId', 'actualOpenAmount', 'invoiceAge', 'age_invoice_y']
        print(data[features])
        # data should be a dataFrame and not a numpy array
        predictions = model.predict(data[features])
        data['predictions'] = predictions
        pred = data.loc[:,['documentNumber','predictions']].to_dict(orient="records")
        return pred
